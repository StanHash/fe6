...
static void sub_800CDB8(struct UnitInfo const* info, struct Unit* unit, int displayMovement);
...
void sub_800CCF0(struct UnitInfo const* info, int displayMovement)
{
    struct Unit* unit;
    ...
    sub_800CDB8(info, unit, displayMovement);
    RefreshEntityMaps();
}

void sub_800CD98(struct UnitInfo const* info, struct Unit* unit)
{
    sub_800CDB8(info, unit, FALSE);
    RefreshEntityMaps();
}
...
static void sub_800CDB8(struct UnitInfo const* info, struct Unit* unit, int displayMovement)
{
    ...
    if (displayMovement && !(unit->state & US_UNDER_A_ROOF))
    {
        TryMoveUnit(unit, info->xLoad, info->yLoad, FALSE);
        RefreshSMS();

        if (info->xLoad != info->xMove || info->yLoad != info->yMove)
            sub_800EEA0(displayMovement, unit, info->xMove, info->yMove);
    }
    else
    {
