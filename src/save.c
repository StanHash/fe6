...
void SramInit()
{
    ...
    SetSramFastFunc();
    REG_IE |= 0x2000;
    WriteSramFast((u8*)(&buf[0]), gUnk_08689DD8 + 0x6EF4, 4);
    (*ReadSramFast)(gUnk_08689DD8 + 0x6EF4,(u8*)(&buf[1]), 4);
...
void EraseSaveData()
{
    ...
    for (i = 0; i < 0x200; i++)
        WriteAndVerifySramFast((u8*)buf, gUnk_08689DD8 + i * 0x40, 0x40);
}
...
bool LoadGlobalSaveInfo(struct GlobalSaveInfo *saveInfo)
{
    ...
    (*ReadSramFast)((u8*)gUnk_08689DD8, (u8*)info, sizeof(struct GlobalSaveInfo));

    if ((0 != StringEquals((char*)info, gUnk_08327314)) &&
        (0x11217 == info->magic_11217) &&
        (0x200A == info->magic_200A) &&
...
void SaveGlobalSaveInfo(struct GlobalSaveInfo *saveInfo)
{
    ...
    WriteAndVerifySramFast((u8*)saveInfo, gUnk_08689DD8, sizeof(struct GlobalSaveInfo));
}
